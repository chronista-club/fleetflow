...
### Phase 2: 拡張機能（Issue #7） ✅ 完了
- [x] include ディレクティブ対応
  - [x] 相対パス解決
  - [x] グロブパターン対応（`services/*.kdl`）
  - [x] 循環参照チェック
  - [x] ネストしたinclude対応
  - [x] エラーハンドリング
- [x] 変数展開対応
  - [x] 変数定義のパース（`variables { }` ブロック）
  - [x] テンプレート展開（Tera使用）
  - [x] 環境変数からの読み込み（FLOW_*, CI_*, APP_*）
  - [x] 未定義変数のエラーハンドリング
- [x] ユニットテスト (7件追加、合計48件)
  - [x] 変数展開のテスト (3件)
  - [x] includeのテスト (4件)
- [x] KDL構文リファレンス作成（spec/07-kdl-syntax-reference.md）
...
### 2025-11-23: Phase 2拡張機能の実装完了
- **理由**: Issue #7の完了
- **影響**:
  - include機能: 再帰的読み込み、グロブパターン、循環参照チェック
  - 変数展開機能: Teraテンプレート、環境変数サポート
  - テスト7件追加（合計48件）
- **コミット**: 763a0f8, 76468c5
