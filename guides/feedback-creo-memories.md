...
**症状**:
- `fleetfleetflow up` 後、コンテナ間でホスト名解決ができない
...
**期待される動作**:
1. `fleetfleetflow up --stage local` でネットワーク自動作成
2. 定義されたサービスを自動接続
3. サービス名でのDNS解決が可能
4. `fleetfleetflow down` でネットワークも削除
...
2. **リモートデプロイ**
   ...
   fleetfleetflow up --stage dev --remote
   ```
...
`local` / `dev` / `prod` のステージ分離が明確で、環境ごとの設定差分が管理しやすい。
