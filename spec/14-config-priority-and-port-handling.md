...
1.  **基盤ロード (`include` / `workload`)**: 
    - 共通テンプレートやワークロード定義を最初に読み込み、基本となるサービス構成を構築します。
2.  **プロジェクトロード (`flow.kdl`)**: 
    - プロジェクト共通の設定を適用し、基盤設定を具体化・上書きします。
3.  **ステージロード (`flow.{stage}.kdl`)**: 
    - 特定の実行環境 (dev/prod 等) に固有の設定を適用します。
4.  **ローカルロード (`flow.local.kdl`)**: 
...
- 意図的にポートを占有させた状態から `fleetflow up` が成功することを確認する。
