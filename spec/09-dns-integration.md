...
```bash
fleet cloud down --stage live --yes
```
...
```bash
# 現在のDNSレコード一覧
fleet cloud dns list

# 手動でレコード追加
fleet cloud dns add --subdomain mcp-prod --ip 203.0.113.1

# 手動でレコード削除
fleet cloud dns remove --subdomain mcp-prod
```
